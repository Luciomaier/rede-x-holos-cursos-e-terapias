# Acordo Rede Publicidade × Holos

> **Fonte da verdade** da parceria comercial/técnica entre o Rede Publicidade (Lucio + Elis + Nick) e a Holos Cursos e Terapias.
> *(Marca no masculino — decisão do Lucio, 07/07/2026: "o Rede", como "O Boticário".)*
>
> **Os quatro documentos desta pasta:**
> - `acordo-rede-holos.md` (este) — **o quê** está contratado: escopo, valores, status
> - `o-que-a-holos-recebe.html` — **o que a Holos recebe**: apresentação de valor para a Luciana
> - `gabarito-operacional.html` — **como** a operação funciona: manual completo (equipe, fluxo, grade de conteúdo, funil, roadmap LPs, 5 mensagens, checklists semanais, reuniões, métricas, canal Gi)
> - `checklists-diarios.html` — **o dia a dia**: rotina e checklist por pessoa (Lucio, Nick, Elis), dia da semana a dia da semana
>
> Quando os números/escopo mudarem aqui, atualizar os três HTML que derivam deste documento.

**Status:** 🟡 Proposta APRESENTADA à Luciana — martelo não bateu; a Lu está adiando as conversas de fechamento
**Assinatura prevista:** Julho/2026 (em risco pelo adiamento)
**Última atualização:** 07/07/2026

---

## 📊 Números oficiais (fonte da verdade)

> Definidos com Lucio + Elis em 15/06/2026. Estes são os números que valem — qualquer divergência nos HTML deve ser corrigida para bater com esta seção.

### Funil de matrículas

```
~300–350 conversas/mês  (topo — medido no ZenPro)
            ↓
   35 matrículas/mês     (maio/2026 real)  →  META 50/mês
```

**Matrículas — formação em massoterapia (por forma de pagamento):**

| Tipo | Como paga | Vem de | Maio/2026 (real) | Meta/mês |
|------|-----------|--------|------------------|----------|
| **Integral** | cartão / à vista | Flex ou Presencial | 15 | 25 |
| **Mensalista** | mensalidade | Presencial (às vezes Flex) | 20 | 25 |
| **TOTAL** | | | **35** | **50** |

- **Baseline:** maio/2026 fechado (35). **Junho parcial (15/06):** 10 integral + 3 mensalista = 13 → integral em bom ritmo, mensalista atrás.
- **Topo do funil:** ~300–350 conversas/mês no ZenPro. O número "180" dos azulejos era um retrato do começo do mês (180 conversas já iniciadas naquele dia) — **descartado como métrica**.
- **Taxa de conversão real:** ~11% (35/325) · **meta:** ~15% (50/325).
- **EAD e Essencial:** metas **sugestivas** — campanhas ainda não rodando. Entram quando ligarem.
- **Número "34" (8+7+11+8 por produto):** **superado** — era divisão por produto que não reflete como se vende. A verdade é por forma de pagamento (50).

### Meta de faturamento — Junho/2026 (planilha da Elis, snapshot 15/06)

Meta mensal total: **R$ 195.101,55** · Realizado no snapshot: **R$ 60.151,77 (~31%)** · Falta: **R$ 134.949,78** *(mês em andamento — normal estar em ~31% no meio do mês)*

| Produto | Meta qtd | Meta R$ | Vend. qtd | Vendido R$ | % atingido |
|---------|----------|---------|-----------|------------|------------|
| Massoterapia 1ª mensalidade *(= mensalista)* | 25 | 10.000,00 | 3 | 2.700,00 | 27,0% |
| Massoterapia Integral, Intensivo *(= integral)* | 20 | 72.000,00 | 9 | 26.023,18 | 36,1% |
| Massoterapia Intensivo *(= integral)* | 5 | 18.000,00 | 1 | 2.834,17 | 15,8% |
| Mensalidade sem a 1ª *(recorrência)* | 120 | 30.000,00 | 42 | 15.415,67 | 51,4% |
| Mensalidade acupuntura | — | 13.000,00 | 9 | 3.483,17 | 26,8% |
| Mensalidade kabbalah | — | 7.000,00 | 17 | 1.657,00 | 23,7% |
| Aluguel de Salas | — | 3.000,00 | 17 | 1.233,00 | 41,1% |
| Aluguel apto Lu | — | 2.101,55 | — | — | 0,0% |
| Atendimento Estágio Social | — | 3.000,00 | — | 476,00 | 15,9% |
| Atendimento Indigo | — | 2.000,00 | 1 | 380,00 | 19,0% |
| Cursos Livres (comercial 1 — Lu, Leo, Laura) | — | 10.000,00 | 6 | 3.073,96 | 30,7% |
| Cursos Livres (comercial 2 — Giih) | — | 15.000,00 | 3 | 2.875,62 | 19,2% |
| Cursos Livres (recepção) | — | 10.000,00 | — | — | 0,0% |
| **TOTAL** | | **195.101,55** | | **60.151,77** | **~31%** |

**Leitura:**
- A planilha **confirma a meta de matrículas**: 1ª mensalidade (25) + Integral (20) + Intensivo (5) = **25 mensalista + 25 integral = 50** ✅. Vendido em junho: 3 + (9+1) = 13, igual ao informado.
- **Massoterapia nova = R$100.000/mês** de meta (R$10k mensalistas + R$72k integral + R$18k intensivo) — bate com a apresentação.
- O restante da receita (R$95k) vem de: **recorrência** (mensalidades já ativas, acupuntura, kabbalah), **aluguéis** (salas + apto), **atendimentos** (estágio social, índigo) e **cursos livres** (3 canais comerciais).
- Snapshot tirado no meio do mês — os % baixos são esperados (mês ainda rodando).

### Landing pages (5)

LP EAD · LP Essencial · LP Flex · LP **Presencial** (turmas 6 e 12 meses) · LP **Mensalistas** (nova).

### ⚠️ CRM: ZenPro, não PulsarZap

O CRM em uso na operação da Holos é o **ZenPro**. O **PulsarZap não entra no circuito agora**. Os HTML (gabarito, checklists, apresentação) citavam "PulsarZap" — ✅ **corrigidos para ZenPro em 17/07/2026** (os 3 HTML + as linhas deste próprio documento, que também citavam PulsarZap no pacote e no "quem faz o quê").

---

## 🔴 PENDÊNCIA ABERTA — Calibração (continuação)

> ✅ **Resolvido em 15/06:** meta de matrículas (50/mês) + topo do funil (300–350 conversas ZenPro) + correção PulsarZap→ZenPro + planilha de faturamento da Elis registrada (meta R$195.101,55/mês). Ver seção "Números oficiais" acima.
>
> **Ainda em aberto:**
> 1. **Cadência do relatório p/ Luciana:** semanal, quinzenal ou por dezena (10/20/30)? *(os 4 docs divergem)*
> 2. ✅ **Valor "antes" real: RESOLVIDO em 07/07/2026 — R$12.500/mês** (confirmado pelo Lucio). Composição: Elis R$4.500 + tráfego R$3.000 + gestão comercial R$3.000 + comissão ~R$1.000 + Holos Connect R$1.000. As duas versões anteriores estavam erradas (R$10.500 da memória e R$11.750 da apresentação). Bate com a planilha de junho: lado Lucio R$8.000 (3k+3k+1k+1k) + Elis R$4.500 por fora. **Corrigir a coluna "antes" na apresentação HTML antes da próxima conversa com a Lu** — com o antes real, o aumento da proposta é de só **+R$1.000/mês** (argumento mais forte).
>
> **Depois de fechar:** propagar tudo para os 3 HTML + ~~corrigir PulsarZap→ZenPro~~ (✅ feito 17/07) + preencher brancos (horários de reunião, datas das LPs) + padronizar nome (Nicholas/Nick) e cor da marca.
>
> Análise de prontidão: 14/06/2026 — scorecard inicial ~6,5/10. Meta/funil calibrados em 15/06.

---

## Status atual

| Etapa | Situação |
|-------|----------|
| Alinhamento interno (Lucio + Elis + Nick) | ✅ Fechado em 02/06/2026 |
| Apresentação à Luciana | ✅ Apresentada — resposta pendente: **a Lu está adiando as conversas** (registro 07/07/2026) |
| Aval de Vani (VP, 50%, poder de veto) | ⬜ Pendente — risco principal |
| Formalização jurídica (contrato assinado) | ⬜ Prevista Julho/2026 |

---

## O pacote — R$ 13.500/mês fixo

| Serviço | O que inclui | Responsável | Valor/mês |
|---------|--------------|-------------|-----------|
| Tráfego Pago | Campanhas Meta + Google · otimização · UTMs · relatório por dezena | Nick + Lucio | R$ 3.000 |
| Site + LPs + Copy + Email | Landing pages (1/semana) · copy · email marketing · disparos | Nick + Elis | R$ 1.500 |
| Gerência Geral | Coordenação da equipe · reuniões · relatórios · interface com a direção | Elis | R$ 6.000 |
| Holos Connect + CRM ZenPro | Plataforma de alunos · CRM/WhatsApp · rastreamento de leads · infra por conta da Rede (substitui custo variável ~R$992/mês) | Lucio | R$ 1.000 |
| API Oficial Meta `PRIORIDADE` | Migração do número para API Oficial · elimina risco de bloqueio · escala segura (mercado: R$800–3.000) | Lucio + Nick | incluso |
| Social + Edição de Vídeo | 3 Reels/semana · posts de feed · stories diários (Lu grava, Nick edita, Elis publica) | Nick + Elis | R$ 2.000 |
| **TOTAL FIXO** | | | **R$ 13.500** |

**Não incluso:** verba de anúncio (Holos paga direto às plataformas) · gravação de vídeos (Luciana grava) · novas funcionalidades no Holos Connect (orçamento à parte).

---

## Antes × Depois

| Antes (contrato atual) | Depois (proposta Julho/2026) |
|------------------------|------------------------------|
| Tráfego R$ 3.000 — mantém | Tráfego R$ 3.000 — mantém |
| Site + email R$ 1.500 — mantém | Site + email R$ 1.500 — mantém |
| Gestão Comercial R$ 3.000 — **some** | Gerência R$ 6.000 — sobe (Elis valorizada) |
| Lucratividade R$ 3.000 — **some** | Social + edição R$ 2.000 — novo |
| Comissão R$ 1.250 — **some** | API Oficial Meta — incluso |
| Social R$ 0 — entra | Comissão comercial Rede — **zero** |
| | Participação de resultados — *a definir em conjunto* |
| **Total: R$ 11.750 + extras imprevistos** | **Total: R$ 13.500 fixo** |

> ⚠️ **PENDÊNCIA CRÍTICA — confirmar o "antes" real.** A memória do negócio registra o acordo atual como **R$ 10.500/mês**; a apresentação usa **R$ 11.750**. Antes de mostrar à Luciana, confirmar com a Elis exatamente o que a Holos paga hoje e sob qual rótulo. Se a coluna "antes" estiver errada, a Luciana contesta o número ao vivo (repete o atrito do relatório de tráfego de mai/26).

**Readequação de comissão comercial:** Giovanna (Comercial Mongaguá) passa de 10% → 5% — o percentual que ia para a Rede fica na Holos.

---

## Quem faz o quê

- **Lucio** — Google Ads · Holos Connect + CRM ZenPro · API Oficial Meta · estratégia do funil · Google Meu Negócio · diagnóstico de performance
- **Nick** — Meta Ads (todas as campanhas) · relatórios por dezena (dias 10/20/30) · landing pages (1/semana) · copy + email · edição de Reels (3/semana) · UTMs
- **Elis** — coordenação da equipe · alinhamento semanal com Luciana · publicação de conteúdo · repasse de relatórios com contexto · **ponto de contato oficial da Rede**

## Fluxo de informação

```
Luciana (Holos) → Elis (ponto de contato) → Lucio / Nick (execução)
```

| Situação | Quem aciona | O que acontece |
|----------|-------------|----------------|
| Poucos leads / telefone fraco | Luciana → Elis | Elis consulta Lucio → análise com dados → resposta estruturada |
| Problema técnico (plataforma/bot) | Elis → Lucio | Lucio resolve + prazo → Elis informa Luciana |
| Solicitação nova (fora do escopo) | Luciana → Elis | Equipe avalia e orça à parte |
| Relatório de desempenho | Nick → Elis | Dias 10/20/30 · Elis repassa com contexto |

---

## Pendências antes de apresentar

- [ ] **Confirmar a coluna "antes" real** (R$10.500 vs R$11.750) — Elis
- [ ] Corrigir numeração das páginas no HTML (6 primeiras dizem "de 07", são 8 páginas)
- [x] Padronizar a marca no HTML — **DECIDIDO (07/07/2026): "o Rede" (masculino, R maiúsculo)**; aplicar nos 3 HTML
- [ ] Alinhar meta do funil (50 matrículas/mês aqui vs "180 conversões × 12" no Funil Documento 2)
- [ ] Decidir: a call é só com Luciana ou a proposta precisa sobreviver ao repasse para Vani?
- [ ] Definir se Elis entra no fee da Rede ou tem contrato MEI separado (impacto tributário/jurídico — ver Advogado Briefing no vault)
- [ ] Agendar a apresentação

---

## Riscos

1. **Vani** — VP e co-fundadora com 50% e poder de veto real. Ausente, mas decide. Pergunta jurídica em aberto: assinatura só da Luciana basta, ou precisa de Vani + Adriano?
2. **Timing emocional** — assinar enquanto o clima é de parceria. Luciana opera no emocional e está em separação conjugal (adiar qualquer cessão de % do ZenPro/UniMasso para ela — entraria na partilha).
3. **Coluna "antes" inconsistente** — ver pendência crítica acima.

---

## Onde mora o quê

- **Apresentação (artefato):** `o-que-a-holos-recebe.html` (nesta pasta) — para mostrar à Luciana
- **Gabarito operacional (manual):** `gabarito-operacional.html` (nesta pasta) — manual completo da operação: equipe, valores, fluxo de informação, grade de conteúdo, funil, roadmap de LPs, as 5 mensagens, checklists semanais, cadência de reuniões, métricas, canal Gi
- **Checklists diários (execução):** `checklists-diarios.html` (nesta pasta) — rotina por pessoa e por dia da semana, derivada do gabarito. É o que cada um abre todo dia
- **Rascunhos e sessões (histórico):** vault Obsidian — `Projetos/Holos/`, `Projetos/Rede Publicidade/`, e `Mapeamentos/Junho 2026/Sessão 02-06-2026 — Alinhamento Holos × Rede Publicidade.md`
- **Briefing jurídico:** vault — `Projetos/Holos/Advogado Briefing.md`
- **Contratos pendentes (checklist):** vault — `Projetos/Holos/Contratos Holos — Pendências.md`
